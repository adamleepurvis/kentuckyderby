    ...
    <div className="space-y-6">
      ...
      {entries.length === 0 ? (
        <div className="bg-white border border-gray-200 rounded-lg p-10 text-center text-gray-500">
          No results yet. Results will appear here once a race is finished and a winner is declared.
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {entries.map((entry, i) => (
                <tr
                  key={i}
                  className={`border-b border-gray-100 ${entry.won ? 'bg-emerald-50' : 'bg-red-50'}`}
                >
